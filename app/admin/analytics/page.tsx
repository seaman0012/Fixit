...
export default async function AnalyticsPage() {
  ...
  const statusStats = [
    ...
    { status: "กำลังดำเนินการ", count: tickets.filter((t: any) => t.status === "in_progress").length },
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground">
          ข้อมูลสถิติและการวิเคราะห์เพื่อการจัดการเชิงรุก
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">รายการแจ้งซ่อมทั้งหมด</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">จากสร้างถึงเสร็จสิ้น</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {topCategories[0]?.count || 0} รายการ
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              สถิติอุปกรณ์ที่เสียบ่อย เพื่อวางแผนการซ่อมบำรุง
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              {topCategories.map((item, index) => (
                <div key={item.category} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-muted-foreground">{item.count} รายการ</span>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              ห้องที่มีปัญหาบ่อย อาจต้องตรวจสอบเชิงรุก
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {topRooms.slice(0, 5).map((item, index) => (
                <div key={item.room} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-muted-foreground">{item.count} ครั้ง</span>
                    <div className="h-2 w-24 overflow-hidden rounded-full bg-muted">
                      <div
                        className="h-full bg-primary"
                        style={{
